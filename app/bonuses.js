...
function letterChecker(str) {
  let missing = "no missing letters";
  for (let i = 0; i < str.length - 1; i++) {
    let currentLetter = str.charCodeAt(i);
    let nextLetterExpected = currentLetter + 1;
    let actualNext = str.charCodeAt([i + 1]);
    if (nextLetterExpected != actualNext) {
      missing = String.fromCharCode(nextLetterExpected);
      break;
    }
  }
  return missing;
